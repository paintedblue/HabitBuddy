...
def add_reference_childof(obj, hand):
    con = obj.constraints.new(type="CHILD_OF")
    con.name = f"Reference_ChildOf_{hand}"
    con.target = arm
    con.subtarget = hand
    con.influence = 0.0


add_reference_childof(brush, "RightHand")
add_reference_childof(tube, "LeftHand")


def key_obj(obj, frame, loc, rot, scale):
...
for frame, loc in brush_locs.items():
    key_obj(brush, frame, loc, brush_rot, brush_scale)

brush_rot_m = Euler(brush_rot, "XYZ").to_matrix().to_4x4()
tube_rot_m = Euler(tube_rot, "XYZ").to_matrix().to_4x4()
nozzle_offset = tube_rot_m @ Vector((0.135, 0, 0))
...
for frame in [1, 12, 24, 36, 48, 56, 64, 72, 80, 92, 108]:
    if frame < 36:
        ...
        site = Vector(brush_locs[frame]) + (brush_rot_m @ Vector((-0.135, 0, 0.035)))
    ...
    pulse = {56: (1.14, 0.90, 0.96), 72: (1.15, 0.86, 0.94)}.get(frame, (1, 1, 1))
    scale = tuple(tube_scale_base[i] * pulse[i] for i in range(3))
    key_obj(tube, frame, loc, tube_rot, scale)
