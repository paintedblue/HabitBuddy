...
```sh
gcloud builds submit --config cloudbuild.yaml --substitutions=_REGION=asia-northeast3,_SERVICE_NAME=habit-buddy-api,_ARTIFACT_REPOSITORY=habit-buddy,_CLOUD_SQL_INSTANCE=<GCP_PROJECT_ID>:asia-northeast3:habit-buddy-postgres,_VPC_CONNECTOR=habit-buddy-connector,_PUBLIC_API_BASE_URL=https://<CLOUD_RUN_URL>
```
...
gcloud run services update habit-buddy-api \
  ...
  --update-env-vars=PUBLIC_API_BASE_URL=https://<CLOUD_RUN_URL>,SUNO_CALLBACK_URL=https://<CLOUD_RUN_URL>/songs/callbacks/suno
```
